            ...
            <input type="hidden" name="resId" value="${restaurant.resId}">
              ...
              <table style="margin-left: 300px; margin-top: -260px;">
                <tr>
                  ...
                  <td><input type="text" name="resName" id="resName" value="${restaurant.resName}"></td>
                </tr>
                <tr>
                  <td><i class="fas fa-utensils"></i>&nbsp;&nbsp;&nbsp;<label>가게설명</label></td>
                  <td><textarea id="rContent" name="resContent" style="height: 100px !important; resize: none" readonly="readonly" onclick="modal()">${restaurant.resContent}</textarea></td>
                </tr>
                <tr>
                  <td><i class="fas fa-utensils"></i>&nbsp;&nbsp;&nbsp;<label>전화번호</label></td>
                  <td><input type="tel" name="resPhone" value="${restaurant.resPhone}" id="resPhone" placeholder="예)031-1234-1234"></td>
                </tr>
                <tr>
                  <td><i class="fas fa-utensils"></i>&nbsp;&nbsp;&nbsp;<label>영업시간</label></td>
                  <td><input type="text" name="resTime" id="resTime" value="${restaurant.resTime}" placeholder="예)12:00~22:00"></td>
                </tr>
                
                <tr>
                  <td><i class="fas fa-utensils"></i>&nbsp;&nbsp;&nbsp;<label>메뉴1</label></td>
                  <td><input type="text" name="menu" id="menu1" value="${rMenu[0] }"></td>
                </tr>
               
                <tr>
                  <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label>메뉴2</label></td>
                  <td><input type="text" name="menu" id="menu2" value="${rMenu[1] }"></td>
                </tr>
               
                <tr>
                  <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label>메뉴3</label></td>
                  <td><input type="text" name="menu" id="menu3" value="${rMenu[2] }"></td>
                </tr>
                ...
                <tr>
                  ...
                  <td><input type="radio" name="resMembership" onclick="partnerOff()" id="nRes">일반맛집&nbsp; <input type="radio" id="pRes" name="resMembership" onclick="partnerOn()">파트너맛집</td>
                </tr>
                <tr>
				...
				<c:forTokens items="${restaurant.resAddress }" delims="," var="addr" varStatus="status">
					<c:if test="${status.index eq 0 }">
		                <tr>
		                  <td><label>우편번호</label></td>                  
		                  <td>
		                    <input type="text" name="post" id="post"
		                       class="postcodify_postcode5" value="${addr }" size="6" readonly="readonly">
		           	...
		           	<c:if test="${status.index eq 1 }">
		                <tr>
		                  ...
		                  <td><input type="text" name="address1" readonly="readonly" value="${addr }"
		            ...
		            <c:if test="${status.index eq 2 }">
		                <tr>
		                  ...
		                  <td><input type="text" name="address2" readonly="readonly" value="${addr }"
...
<script>
	...
	$(function(){
	/* div id_100 클래스의  select옵션 의 값을 "val2"의 값으로 한다. */
	/* $("div.id_100 select").val("val2"); */
	var resCategoryId = ${restaurant.resCategoryId};	
	$('#resCategoryId option[value='+resCategoryId+']').attr('selected','selected');
	
	var locationId = ${restaurant.locationId};	
	$('#locationId option[value='+locationId+']').attr('selected','selected');
	
	var filter = ${rfilter};
	for(var i = 0 ; i < filter.length; i++){
		$('input:checkbox[name="filter"]').each(function() {
		     if(this.value == filter[i]){ //값 비교
		            this.checked = true; //checked 처리
		      }
		 });		
	}
	
	if($("#partner").prop("checked")==true){
		$("#pRes").click();
	}else{
		$("#nRes").click();
	}	
